unkData_000_32dd:
; Credits_Producer
	dw $9c02
	dw $9c02 + $20

; Credits_Director
	dw $9c02
	dw $9c02 + $20
	dw $9c02 + $40

; Credits_Programmer
	dw $9c02
	dw $9c02 + $20
	dw $9c02 + $40

; Credits_MusicSound
	dw $9c02
	dw $9c02 + $20

; Credits_Design
	dw $9c02
	dw $9c02 + $20
	dw $9c02 + $40

	dw $9c02 + $20
	dw $9c02 + $40

; Credits_SpecialThanks
	dw $9c02
	dw $9c02 + $20
	dw $9c02 + $40

	dw $9c02 + $20
	dw $9c02 + $40

	dw $9c02 + $20
	dw $9c02 + $40

	dw $9c02 + $20
	dw $9c02 + $40

; Credits_Solarstriker
	dw $9c02
	dw $9c02 + $20
	dw $9c02 + $40
	dw $9c02 + $60

unkData_000_3315:
	dw Credits_Producer
	dw Credits_GunpeiYokoi
	dw Credits_Director
	dw Credits_SatoruOkada
	dw Credits_KeisukeTerasaki
	dw Credits_Programmer
	dw Credits_KenSugimoto
	dw Credits_Blank1
	dw Credits_MusicSound
	dw Credits_TohruOsada
	dw Credits_Design
	dw Credits_AkiHigashiya
	dw Credits_KYamagami
	dw Credits_KKatsuyama
	dw Credits_KHayashi
	dw Credits_SpecialThanks
	dw Credits_HHirayama
	dw Credits_YFujii
	dw Credits_KTakeuchi
	dw Credits_HiroshiWada
	dw Credits_YKawakami
	dw Credits_MNakatani
	dw Credits_HNakazawa
	dw Credits_YArashiba
	dw Credits_Solarstriker
	dw Credits_Blank2
	dw Credits_Blank3
	dw Credits_Copyright

Credits_Producer:		db "PRODUCER@@@@@@@@@"
Credits_GunpeiYokoi:		db "G.YOKOI@@@@@@@@@@"


Credits_Director:		db "DIRECTOR@@@@@@@@@"
Credits_SatoruOkada:		db "S.OKADA@@@@@@@@@@"
Credits_KeisukeTerasaki:	db "K.TERASAKI@@@@@@@"


Credits_Programmer:		db "PROGRAMMER@@@@@@@"
Credits_KenSugimoto:		db "K.SUGIMOTO@@@@@@@"
Credits_Blank1:			db "@@@@@@@@@@@@@@@@@"


Credits_MusicSound:		db "MUSIC@SOUND@@@@@@"
Credits_TohruOsada:		db "T.OSADA@@@@@@@@@@"


Credits_Design:			db "DESIGN@@@@@@@@@@@"
Credits_AkiHigashiya:		db "A.HIGASHIYA@@@@@@"
Credits_KYamagami:		db "K.YAMAGAMI@@@@@@@"

Credits_KKatsuyama:		db "K.KATSUYAMA@@@@@@"
Credits_KHayashi:		db "K.HAYASHI@@@@@@@@"


Credits_SpecialThanks:		db "SPECIAL THANKS TO"
Credits_HHirayama:		db "H.HIRAYAMA@@@@@@@"
Credits_YFujii:			db "Y.FUJII@@@@@@@@@@"

Credits_KTakeuchi:		db "K.TAKEUCHI@@@@@@@"
Credits_HiroshiWada:		db "H.AYRTON.WADA@@@@"

Credits_YKawakami:		db "Y.KAWAKAMI@@@@@@@"
Credits_MNakatani:		db "M.NAKATANI@@@@@@@"

Credits_HNakazawa:		db "H.NAKAZAWA@@@@@@@"
Credits_YArashiba:		db "Y.ARASHIBA@@@@@@@"


Credits_Solarstriker:		db "@SOLARSTRIKER@@@@"
Credits_Blank2:			db "@@@@@@@@@@@@@@@@@"
Credits_Blank3:			db "@@@@@@@@@@@@@@@@@"
Credits_Copyright:
	;	     (©)  1    9    8    9           N  i  n  t  e  n  d  o
	db	$bf, $fc, $a2, $b2, $b0, $b2, $bf, $ea, $eb, $ec, $ed, $ee, $ef, $bf, $bf, $bf, $bf
